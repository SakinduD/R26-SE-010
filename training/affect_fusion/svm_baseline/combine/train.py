"""
Train the MCA RAVDESS+SUBESCO combined SVM Speech Emotion Recognition model.

The saved artifact is a sklearn Pipeline:
    Scaler(Standard or Robust) -> [PCA] -> SVC(probability=True)

It is backend-compatible with SerAnalyzer.EMOTION_MAP in
Backend/app/api/v1/mca/nudge_engine.py. This script does not read or write
anything under Backend/ -- promoting a trained model there is a separate,
explicit step.
...
# Live progress even when stdout is redirected to a log file.
...
# Cap on libsvm's solver iterations so a poorly-conditioned fit can't hang.
SVM_MAX_ITER = 50_000
...
def load_dataset(data_path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]:
    ...
    # Safety net: preprocess.py already discards non-finite rows, but drop
    # any that slip through rather than crash training.
    finite_mask = np.all(np.isfinite(X), axis=1)
    if not finite_mask.all():
        n_bad = int((~finite_mask).sum())
        print(f"WARNING: dropping {n_bad} row(s) with NaN/Inf values (see preprocess.py contract).")
...
def json_safe_params(params: dict) -> dict:
    ...
    for key, value in params.items():
        if isinstance(value, np.generic):
            safe[key] = value.item()
        elif hasattr(value, "get_params"):  # sklearn estimator instance
...
def make_holdout_split(
    ...
    # Speaker-independent test set: actors stay entirely in train or test.
...
def build_search_pipeline() -> Pipeline:
    return Pipeline(
        steps=[
            ...
            ("svm", SVC(class_weight="balanced", probability=False, max_iter=SVM_MAX_ITER)),
        ]
    )
...
def build_final_pipeline(best_params: dict) -> Pipeline:
    ...
    # Fresh unfitted copy of whichever scaler the search selected.
    steps = [("scaler", clone(best_params["scaler"]))]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a RAVDESS+SUBESCO combined MCA SVM SER model.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=-2,
        help="Parallel worker processes for the search. Default: -2 (all CPU cores but one, "
        "so the machine stays responsive). Use -1 for all cores, or a small positive int "
        "if memory is tight (each worker holds its own copy of the training fold).",
    )
    parser.add_argument(
        "--no-group-split",
        action="store_true",
    ...
    print("=== MCA SER Training: RAVDESS+SUBESCO combined SVM ===")
    print(f"Loaded {X.shape[0]} samples, {X.shape[1]} features")
    print(f"svm__max_iter cap: {SVM_MAX_ITER} (guarantees every fit terminates)")
    ...
    param_dist = {
        # RobustScaler included alongside StandardScaler since MFCC/mel
        # energy stats can have heavy-tailed outliers.
        "scaler": [StandardScaler(), RobustScaler()],
        "pca__n_components": [0.90, 0.95, 0.98, None],
        # C kept below 50 to avoid slow-to-converge hard-margin fits.
        "svm__C": loguniform(0.01, 50.0),
    ...
    search = RandomizedSearchCV(
        ...
        refit=True,
        random_state=RANDOM_STATE,
        error_score=np.nan,  # bad candidate scores NaN instead of aborting the search
    )

    total_fits = args.n_iter * cv.get_n_splits()
    print(f"\n[1/3] Hyperparameter search ({args.n_iter} candidates x {cv.get_n_splits()} folds = {total_fits} fits, macro F1)...")
    try:
        search.fit(X_train, y_train, groups=fit_groups)
    except MemoryError:
        print(
            "\nOut of memory during the search. Try: a smaller --n-jobs "
            "(each worker holds its own copy of the training fold), or "
            "dropping the 'None' (no-PCA) option from pca__n_components "
            "above, which is the most memory-hungry candidate on large "
            "datasets."
        )
        raise
    except KeyboardInterrupt:
        print("\nInterrupted by user -- no model was saved.")
        raise

    if not np.isfinite(search.best_score_):
        raise RuntimeError(
            "Every hyperparameter candidate failed (best_score_ is NaN). "
            "Check the warnings above for the actual per-candidate errors."
        )

    print(f"Best CV macro F1: {search.best_score_:.4f}")
    print(f"Best parameters : {json_safe_params(search.best_params_)}")
